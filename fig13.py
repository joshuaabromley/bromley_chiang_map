...
p1 = 0.95 ##[0,1] for pierrehumbert, [1,10] for Guillot
p2 = 38 ##[20,40]
...
ax[1][0].text(105,1e-5, "$p_1 = {0}$".format(p1), fontsize = textSize)
ax[1][0].text(105,1e-6, "$p_2 = {0}$".format(p2), fontsize = textSize)
ax[1][0].text(105,1e-7, "$p_3 = {0}$".format(p3), fontsize = textSize)
ax[1][0].text(105,1e-8, "$p_4 = {0}$".format(p4), fontsize = textSize)
ax[1][0].text(105,1e-9, "$\lambda = ${0:.4f}".format(lyExp), fontsize = textSize)
ax[1][0].text(30,5e-12*np.exp(30*lyExp), "$\delta \\tau = \delta \\tau_0 e^{\lambda i}$", fontsize = textSize, rotation = 28)
...
bins,_,__ = ax[1][1].hist(gammaTau[100:], 100, color = "black")
...
ax[1][1].set_xlim(-5,130)
ax[1][1].set_ylim(0.7,1000)
xTicks = np.arange(0,2.8,0.5)
